...
def buy_rune(order, private_key, gas_price):
...
def print_header(stdscr, start_time):
  stdscr.addstr(0, 0, "开始查询RUNES信息(按'q'键退出)", curses.A_BOLD)
...
def main(stdscr):
  # 读取配置
  with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), '_buy_rune_env'), 'r') as file:
  ...
  while True:
    ...
    for rune in runes:
      rune_id = rune['id']
      min_price = int(rune['minPrice'])
      min_price_eth = Web3.from_wei(min_price, 'ether')
      target = targets_by_id[rune_id]
      ...
      log = f"[rune_id={rune_id},order_id={order_id}]最低价为{min_price_eth}，低于{limit_price_eth}"
      if (buy_count >= max_buy):
        continue
      log = f"{log},符合购买条件，发送订单"
      buy_log.append(log)
      try: 
        tx_receipt = buy_rune(order, private_key, gas_price)
